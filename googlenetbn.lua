local function inception(input_size, config)
   ...
   if config[1][1] ~= 0 then
      ...
      conv1:add(nn.SpatialConvolution(input_size, config[1][1],1,1,1,1)):add(nn.SpatialBatchNormalization(config[1][1],1e-3)):add(nn.ReLU(true))
      ------bn
      concat:add(conv1)
   end

   local conv3 = nn.Sequential()
   conv3:add(nn.SpatialConvolution(  input_size, config[2][1],1,1,1,1)):add(nn.SpatialBatchNormalization(config[2][1],1e-3)):add(nn.ReLU(true))
   conv3:add(nn.SpatialConvolution(config[2][1], config[2][2],3,3,1,1,1,1)):add(nn.SpatialBatchNormalization(config[2][2],1e-3)):add(nn.ReLU(true))
   concat:add(conv3)

   local conv3xx = nn.Sequential()
   conv3xx:add(nn.SpatialConvolution(  input_size, config[3][1],1,1,1,1)):add(nn.SpatialBatchNormalization(config[3][1],1e-3)):add(nn.ReLU(true))
   conv3xx:add(nn.SpatialConvolution(config[3][1], config[3][2],3,3,1,1,1,1)):add(nn.SpatialBatchNormalization(config[3][2],1e-3)):add(nn.ReLU(true))
   conv3xx:add(nn.SpatialConvolution(config[3][2], config[3][2],3,3,1,1,1,1)):add(nn.SpatialBatchNormalization(config[3][2],1e-3)):add(nn.ReLU(true))
   concat:add(conv3xx)
   ...
   if config[4][2] ~= 0 then
      pool:add(nn.SpatialConvolution(input_size, config[4][2],1,1,1,1)):add(nn.SpatialBatchNormalization(config[4][2],1e-3)):add(nn.ReLU(true))
   end
   concat:add(pool)
...
function createModel(nGPU)
   ...
   features:add(nn.SpatialConvolution(3,64,7,7,2,2,3,3)):add(nn.SpatialBatchNormalization(64,1e-3)):add(nn.ReLU(true))
   ------bn
   features:add(nn.SpatialMaxPooling(3,3,2,2):ceil())
   features:add(nn.SpatialConvolution(64,64,1,1)):add(nn.SpatialBatchNormalization(64,1e-3)):add(nn.ReLU(true))
   ------bn
   features:add(nn.SpatialConvolution(64,192,3,3,1,1,1,1)):add(nn.SpatialBatchNormalization(192,1e-3)):add(nn.ReLU(true))
   ------bn
   features:add(nn.SpatialMaxPooling(3,3,2,2):ceil())
   ...
   features:add(nn.SpatialBatchNormalization(576,1e-3)) ------bn
   ...
   main_branch:add(nn.SpatialBatchNormalization(1024,1e-3)) ------bn
   ...
   aux_classifier:add(nn.SpatialBatchNormalization(128,1e-3)) ------bn
